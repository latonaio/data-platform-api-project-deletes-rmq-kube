...
import (
	...
	"data-platform-api-project-deletes-rmq-kube/config"
...
func (c *DPFMAPICaller) deleteSqlProcess(
	...
	var projectData *dpfm_api_output_formatter.Project
	for _, a := range accepter {
		switch a {
		case "Project":
			h := c.projectDelete(input, output, log)
			projectData = h
			if h == nil {
				continue
			}
...
func (c *DPFMAPICaller) projectDelete(
	input *dpfm_api_input_reader.SDC,
	output *dpfm_api_output_formatter.SDC,
	log *logger.Logger,
) *dpfm_api_output_formatter.Project {
	sessionID := input.RuntimeSessionID
	project := c.ProjectRead(input, log)
	project.IsMarkedForDeletion = input.Project.IsMarkedForDeletion
	res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": project, "function": "ProjectProject", "runtime_session_id": sessionID})
	if err != nil {
	...
	if !checkResult(res) {
		...
		output.SQLUpdateError = "Project Data cannot delete"
		return nil
	}
